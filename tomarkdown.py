...
def rewriteToMd(soup):
    ...
    subti = soup.find('div', class_='dcr-1oosqmb') or soup.find('div', class_='dcr-95in8m') or soup.find('div', class_='dcr-xq41iu') or soup.find('div', class_='dcr-cj0iop')
    ...
    for tag in tags:
        if type(tag) is not str:
            if tag.name == 'blockquote':
                ...
                stuff.append(f"![|100%]({tag.get('src')})\n")
